...
export default {
    ...
    computed: {
        ...
        calcTablePadding() {
            if (this.tablePadding === undefined) {
                return this.$sstTable.tablePadding || 0
            }
            return this.tablePadding || 0
        },
        calcPagerOffsetHeight() {
            return this.pagerOffset || this.$sstTable.pagerOffset
        },
        calcCustomClassName() {
            return this.customClassName || this.$sstTable.customClassName
        }
    },
    watch: {
        merge() {
    ...
    methods: {
        resize(immediate) {
            const _resizeHeight = () => {
                this.tableHeight = this.$refs.sstTableRef.clientHeight - this.calcPagerTotalHeight
            }
            if (immediate === true) {
                _resizeHeight()
                return
